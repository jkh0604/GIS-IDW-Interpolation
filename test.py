...
class SpatiotemporalApp:
    def __init__(self):
        ...
        self.root.title("Spatiotemporal Data Import and Visualization")
    ...
    def perform_interpolation_wrapper(self):
        if self.data is not None:
            perform_interpolation(self.data, r"C:\Users\17708\Documents\School\Spring 2023\GIS\Project\county_xy.txt") #blkgrp_xy.txt
        else:
        ...
        if file_path:
            ...
            for col in data.columns[2:]:
                if col.startswith("n"):
                    ...
                    # Convert predictions to a NumPy array with a proper data type
                    predictions = predictions.to_numpy(dtype=np.float64)

                    # Calculate the nearest true_value index for each row in the data DataFrame
                    dist_matrix = cdist(locations[['x', 'y']], trueData[['x', 'y']])
                    nearest_indices = np.argmin(dist_matrix, axis=1)

                    # Extract the true_values from the trueData DataFrame using the calculated indices
                    filtered_trueData = trueData.iloc[nearest_indices].reset_index(drop=True)
